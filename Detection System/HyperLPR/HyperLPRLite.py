...
class LPR():
    def __init__(self, model_detection='model/cascade.xml', model_finemapping="model/model12.h5", model_seq_rec="model/ocr_plate_all_gru.h5"):  # model/ocr_plate_all_gru.h5
    ...
    def detectPlateRough(self,image_gray,resize_h = 720,en_scale =1.08 ,top_bottom_padding_rate = 0.05):
        ...
        image_color_cropped = image_gray[padding:resize_h-padding, 0:image_gray.shape[1]]    # image
    ...
    def fastdecode(self,y_pred):
        ...
        results = ""
        confidence = 0.0
        table_pred = y_pred.reshape(-1, len(chars)+1)  # er wei shu zu
        res = table_pred.argmax(axis=1)
        is_exit = False
        for i, one in enumerate(res):
            if one<len(chars) :    # and (i == 0 or one != res[i - 1])
                if is_exit and one >= 0 and one <= 30:
                    continue
                if one >= 0 and one <= 30:
                    is_exit = True
                results += chars[one]
    ...
    def finemappingVertical(self, image, rect):
        resized = cv2.resize(image, (66, 16))   # 66 * 16
    ...
    def recognizeOne(self,src):
        x_temp = src
        x_temp = cv2.resize(x_temp,(164,48))
        x_temp = x_temp.transpose(1, 0, 2)
        #x_temp = x_temp[1:50,:]
        # x_temp = cv2.resize(x_temp,(48,164))
        #cv2.imshow('card', x_temp)
